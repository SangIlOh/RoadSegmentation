...
class build_refine_model(object):
    ...
    def __init__(self, num_channel, num_class, output_HW): # inputs: segmentation result from the first network, targets: final segmentation result
        ...
        with tf.name_scope(_model_name ):
    ...
    def train(self, data, output_path, epochs = 100, max_iter = 10, display_step = 1, opt_kwargs = {}):
        ...
        self._refiner_variables = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = "refine_GAN")
        self._saver = tf.train.Saver(self._refiner_variables)

        self._premodel_variables = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = "refine_GAN/deeplab_v2")
        ...
        with tf.Session() as sess:
            ...
            verification_input = sess.run( self._predictor, feed_dict = { self._x: verification_x, self._keep_prob : np.float32( 1), self._is_train: False})
    ...
    def generator_output_test( self, sess, gan_cost, l1_cost, data):
        ...
        for nd in range( data.num_examples):
            ...
            input0 = sess.run( self._predictor, feed_dict = { self._x: x0, self._keep_prob : np.float32( 1), self._is_train: False})
