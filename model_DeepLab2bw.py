...
#    return tf.nn.conv2d_transpose( x, W, batch_size + output_shape[ 1 :], strides = [ 1, stride, stride, 1], padding = "VALID")
...
def prelu( x, alpha0, name = None):
    
    x_shape = [ dimension.value for dimension in x.get_shape()]
    x_reshape = tf.reshape( x, shape = [ -1, np.prod( x_shape[ 1 :])])
    initial = tf.constant( alpha0, shape = [ np.prod( x_shape[ 1 :])], dtype = tf.float32)
    alphas = tf.Variable( initial)
    
    pos = tf.nn.relu( x)
    neg = alphas * ( x_reshape - abs( x_reshape)) * 0.5

    return tf.add( pos, tf.reshape( neg, shape = [ -1] + x_shape[ 1 :]), name = name)
...
class Refinenet( object):
    ...
    _model_name = "Refinenet"

    def __init__( self, num_layer, num_feature_root, num_channel, num_class, output_HW, is_training = False, filter_size = 3, pool_size = 2, head_name_scope = "Refinenet", opt_kwargs = {}):
        
        """
        use_conv_at_down_sample = opt_kwargs.pop( "use_conv_at_down_sample", False)
        """
        use_conv_at_down_sample = opt_kwargs.pop( "use_conv_at_down_sample", False)
        ...
        with tf.name_scope( head_name_scope):
            if num_feature_root % 2 != 0:
                raise ValueError( "wrong parameter, nl : %d, in_feature : %f" % ( nl, num_feature_root / 2))
    ...
    def train( self, data, output_path, training_iters = 10, epochs = 100, keep_prob = 0.75, display_step = 1, opt_kwargs = {}):
        ...
        logging_str = [ "train_parmas >>\n",
                        "\t\t\tlogging name : {0}\n".format( logging_name),
                        "\t\t\tnum_layer : {0}\n".format( self._num_layer),
                        "\t\t\tnum_feature_root : {0}\n".format( self._num_feature_root),
                        "\t\t\tnum_channel : {0}\n".format( self._num_channel),
                        "\t\t\tnum_class : {0}\n".format( self._num_class),
                        "\t\t\toutput_HW : {0}\n".format( self._output_HW),
                        "\t\t\tfilter_size : {0}\n".format( self._filter_size),
                        "\t\t\toutput_path : {0}\n".format( output_path),
